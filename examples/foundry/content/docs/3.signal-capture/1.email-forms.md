---
title: Email Forms
description: ConvertForm — dynamic field definitions, anti-spam protection, and the submission flow.
---

## Basic Usage

The simplest email capture form:

```mdc
::convert-form{location="hero"}
::
```

This renders a single email field with a "Get Access" button. On submit, the email is delivered via webhook and a success toast is shown.

## Props

| Prop | Type | Default | Description |
|---|---|---|---|
| `fields` | `FieldDef[]` | `[{ name: 'email', type: 'email', ... }]` | Form field definitions |
| `location` | `string` | **required** | Location identifier for tracking |
| `submitLabel` | `string` | `'Get Access'` | Submit button text |
| `note` | `string` | — | Small text below the form |
| `layout` | `'stacked' \| 'horizontal'` | `'stacked'` | Form layout direction |
| `successRedirect` | `string \| false` | — | Redirect path after submission, or `false` to disable redirect |
| `offerSlug` | `string` | — | Offer identifier for tracking and redirect |

## Custom Fields

Define custom fields to capture more than just email:

```mdc
::convert-form
---
location: apply-form
submitLabel: Apply Now
layout: stacked
fields:
  - name: name
    label: Full Name
    type: text
    placeholder: Jane Smith
    required: true
  - name: email
    label: Email
    type: email
    placeholder: jane@example.com
    required: true
  - name: message
    label: Why are you interested?
    type: textarea
    placeholder: Tell us about your project...
    required: false
---
::
```

### FieldDef Shape

| Field | Type | Description |
|---|---|---|
| `name` | `string` | Field identifier (used as form data key) |
| `label` | `string` | Display label above the input |
| `type` | `'text' \| 'email' \| 'textarea' \| 'tel'` | Input type |
| `placeholder` | `string` | Placeholder text |
| `required` | `boolean` | Whether the field must be filled |

## Layouts

### Stacked (default)

Fields stack vertically. Best for multi-field forms:

```mdc
::convert-form{location="apply" layout="stacked" submit-label="Apply"}
::
```

### Horizontal

Email field and button side by side. Best for single-field capture:

```mdc
::convert-form{location="hero" layout="horizontal" submit-label="Get Early Access"}
::
```

## Anti-Spam Protection

Every form submission includes three layers of anti-spam protection, handled automatically by `useFormCapture`:

1. **Honeypot** — A hidden field that bots fill but humans don't. If filled, the form silently redirects (bot thinks it succeeded).

2. **Time-on-form** — Tracks how long the form was visible. Submissions under 2 seconds are flagged as suspicious.

3. **JS token** — A `crypto.randomUUID()` generated on mount. Bots without JavaScript cannot produce a valid UUID.

These are sent with every submission and validated server-side. See [Anti-Spam](/docs/modules/events#anti-spam) for the server-side scoring system.

## Submission Flow

1. User fills form and clicks submit
2. **Client-side validation** — Dynamic Zod schema validates fields
3. **Honeypot check** — If honeypot is filled, silently redirect (bot trap)
4. **Event fired** — `form_submitted` event with formData + antiSpam payload
5. **Webhook provider** — Posts to `/api/v1/webhook` server handler
6. **Server processing** — Anti-spam scoring, rate limiting, webhook delivery
7. **Success** — Shows a success toast (or redirects if `successRedirect` is set)
8. **On error** — Toast notification + `form_error` event fired

## Success Behavior

When a form submits successfully:

1. If `successRedirect` is a string path — navigates to that path
2. If `successRedirect` is `false` — stays on the page, shows a success toast
3. If `successRedirect` is not set (default) — shows a success toast (no redirect)

## Using in Vue Templates

```vue
<ConvertForm
  location="pricing-page"
  submit-label="Start Free Trial"
  layout="horizontal"
  offer-slug="trial"
  :fields="[
    { name: 'email', type: 'email', placeholder: 'you@company.com', required: true },
  ]"
/>
```
