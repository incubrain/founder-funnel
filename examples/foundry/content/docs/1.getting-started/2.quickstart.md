...
- [Node.js](https://nodejs.org){target="_blank"} 22 or later
- A package manager — [pnpm](https://pnpm.io){target="_blank"} is recommended, npm works too
...
The CLI uses [`@nuxt/cli`](https://github.com/nuxt/cli){target="_blank"} under the hood. You can also pass `--template default` explicitly, though `default` is the only template currently available.
::
