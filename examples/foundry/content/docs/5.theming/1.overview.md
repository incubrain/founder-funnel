...
- [Tailwind CSS v4](https://tailwindcss.com/docs/v4-beta){target="_blank"} — Utility-first CSS with the new CSS-first configuration
...
Foundry uses the standard [Nuxt UI color system](https://ui.nuxt.com/getting-started/theme#colors) with semantic color tokens. The primary colors are configured via oklch values in `theme.css` and automatically generate light/dark mode variants.

For full details on available colors and customization options, see the [Nuxt UI Color Documentation](https://ui.nuxt.com/getting-started/theme#colors).
