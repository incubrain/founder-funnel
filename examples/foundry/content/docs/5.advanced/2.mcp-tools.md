...
## Quick Start: Audit Your Docs With an Agent

Paste this into your agent to walk it through the Foundry MCP tools and have it audit your published documentation:

::prompt
---
description: Audit a Foundry-powered site's documentation with the MCP tools.
icon: i-lucide-bot
actions:
  - copy
  - cursor
  - windsurf
---
You are auditing a docs site powered by IncuBrain Foundry. The site exposes four MCP tools:
list-pages, get-page, list-decisions, get-decision. Each returns a contentHash so you can
detect changes without re-fetching everything.

1. Call list-pages to get every documentation page.
2. For each page that looks important to a founder validating a product idea, call get-page
   and read the markdown.
3. Flag pages where the structure is incomplete (missing prerequisites, examples, or
   "next steps" links) and write a concrete suggestion.
4. Cross-reference with list-decisions to see if the founder has logged a strategic
   decision that contradicts what the docs claim.
5. Output a punch list grouped by severity. Cite paths, not titles, so I can navigate quickly.
::
