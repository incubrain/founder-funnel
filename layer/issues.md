...
- [x] replace all `NUXT_SITE_URL` usage with `NUXT_PUBLIC_SITE_URL`
...
- [x] flickering of layout in between layout navigation — fixed by removing eager context clearing in useContentPage and adding page/layout transition CSS
...
- [ ] citations error message is cutoff in popover/tooltip and should be copyable AND appended to the signal buffer as a `kind: 'log'` error row so our AI agents can see what is happening... 
...
- [x] we need at least 5, maybe 10 quality tailwing utility classes for background patterns that hook into the nuxt/ui theming system so users can override the colors
- [x] similar to the pattern task above, we need the same for background gradients
